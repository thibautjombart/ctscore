...
For prioritising individuals,
[`add_ctscore()`](thibautjombart.github.io/ctscore/reference/add_ctscore.md)
attaches the scores onto the `linelist` of the `ctdata` object:
...
to flatten the `ctdata` object into a single data frame, joining the
`risk` table onto each exposure. The argument `by_contact` indicates
whether to return one row per contact (TRUE) or one row per exposure
(FALSE, default):
...
We can simulate contact tracing data using the
[`sim_ctdata()`](thibautjombart.github.io/ctscore/reference/sim_ctdata.md)
function. This function generates a synthetic dataset of contacts and
exposures, which can be useful for testing different follow-up
strategies and evaluating the performance of the scoring system. See
[`?sim_ctdata`](thibautjombart.github.io/ctscore/reference/sim_ctdata.md)
...
[`sim_followup()`](thibautjombart.github.io/ctscore/reference/sim_followup.md)
function.
