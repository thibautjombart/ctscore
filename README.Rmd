...
For convenience, we can ask `ctscore ` to return results in two different shapes:

- as a `data.frame` with contact IDs and scores, which is simpler to handle to prioritise individuals
- as a `ctdata` object with an additional column for the scores appended to the 
exposure database
